...
void trans_cloudTocloud(int n,int N, float robot_frame[], float laser_source[], float model[]) {
...
//fill in the data-> the cloud of point of target and the robot tool
	...
	for (i = 0; i < N; i++) {
		modelCoords->data.fl[i * 3] = robot_frame[i * 3];  /// model
		modelCoords->data.fl[i * 3 + 1] = robot_frame[i * 3 + 1];
		modelCoords->data.fl[i * 3 + 2] = robot_frame[i * 3 + 2];
		printf("%f\t\t%f\t\t%f\n", robot_frame[i * 3], robot_frame[i * 3 + 1], robot_frame[i * 3 + 2]);
	}
...
//refinedRobot = robot_frame - repeatedMeanRobot
...
//covariance matrix = (model- mean model)*(robot_frame - mean robot)
